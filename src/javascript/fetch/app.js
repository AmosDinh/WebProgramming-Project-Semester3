...
function getDatetime(){
    ...
    const formattedDate = `${year}-${month.toString().padStart(2, '0')}-${day.toString().padStart(2, '0')} ${minutes.toString().padStart(2, '0')}`;
...
let postsPerUser = {
    'SpaceX':[
        { username: 'SpaceX', content: "Falcon 9 launches SpaceX’s sixth dedicated smallsat rideshare mission – completing our 200th successful launch!", id: "vnufydfoiww", datetime:getDatetime() },
    ...
    'LexFridman':[
        ...
        { username: 'LexFridman', content: "Here's my conversation with Hikaru Nakamura @GMHikaru, one of the best chess players in the world & a top chess streamer. We talk about his games with @MagnusCarlsen & others, plus psychology of chess & the cheating scandal. ", id: "sdbvruietty", datetime:getDatetime() },
        { username: 'LexFridman', content: "Sometimes a kind word from a stranger can make me completely forget whatever concerns were weighing heavy on my heart. Kindness like that is a small gift that can make the biggest difference. I was lucky to get that today. I'll try to pass it on tomorrow.", id: "sbdsbddfjkbsukjdfa", datetime:getDatetime() },
...
app.post("/api/birdpost", (req, res) => {
    ...
    postsPerUser[req.query.user].push({ username: req.query.user, content: req.query.content, id: req.query.uuid, datetime:getDatetime() }); //uuidv4()
